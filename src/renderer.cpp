...
void Renderer::Render(Snake const snake, SDL_Point const &food, SDL_Point const &poisonFood, SDL_Point const &bonusFood) {
  SDL_Rect block;
  ...
  //Render obstacle
  SDL_SetRenderDrawColor(sdl_renderer, 0x00, 0xFF,0xCC, 0x00);
  for (size_t i = 0; i < _obstacle.getObstacle().size(); i++)
  {
    block.x = _obstacle.getObstacle()[i].x * block.w;
    block.y = _obstacle.getObstacle()[i].y * block.h;
    SDL_RenderFillRect(sdl_renderer, &block);
  }
